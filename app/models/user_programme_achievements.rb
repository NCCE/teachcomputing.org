class UserProgrammeAchievements
  ...
  def online_achievements
    @achievements.not_in_state(:dropped).with_category(Activity::ONLINE_CATEGORY).order('created_at ASC')
  end

  def face_to_face_achievements
    @achievements.not_in_state(:dropped).with_category(Activity::FACE_TO_FACE_CATEGORY).order('created_at ASC')
  end
