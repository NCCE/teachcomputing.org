class CsAcceleratorEligibleCoursesForSecondaryCertificateUserJob
  queue_as :default
  ...
  def perform(user_id)
    ...
    if eligible_courses.any?
      achievement = Achievement.create(activity_id: additional_csa_course_activity, user_id: user.id, programme_id: programme.id)
