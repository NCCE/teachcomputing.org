class FutureLearnCourseRunsJob < ApplicationJob
  queue_as :default

  def perform
    course_runs = FutureLearn::Queries::CourseRuns.all

    course_ids = course_runs.map { |r| r.course.uuid }.uniq
    ...
    course_run_ids_hash.each do |course_id, run_ids|
      ...
      run_ids.each do |run_id|
        FutureLearnCourseRunEnrolmentsJob.perform_later(course_id: course_id,
                                                        run_id: run_id)
      end
    end
  end
