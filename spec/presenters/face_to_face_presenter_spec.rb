...
RSpec.describe FaceToFacePresenter do
  ...
  describe('button_url') do
    it { expect(empty_presenter.button_url).to eq('/courses?topic=Face+to+face') }
    it { expect(empty_presenter.button_url({ certificate: 'cs-accelerator' })).to eq('/courses?certificate=cs-accelerator&topic=Face+to+face') }
  end
