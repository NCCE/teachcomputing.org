  ...
  describe '#after_commit callbacks' do
    describe '#schedule_welcome_email' do
      ...
      context 'when CS Accelerator' do
        it 'queues ScheduleProgrammeGettingStartedPromptJob' do
          expect do
            cs_accelerator_enrolment
          end.to have_enqueued_job(ScheduleProgrammeGettingStartedPromptJob).with(user.id, cs_accelerator.id)
        end
        it 'sends CsAcceleratorMailer' do
          expect { cs_accelerator_enrolment }
