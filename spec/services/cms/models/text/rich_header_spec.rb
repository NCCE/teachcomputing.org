...
RSpec.describe Cms::Models::Text::RichHeader do
  ...
  it "should extract text when only one child" do
    hib = Cms::Models::Text::RichHeader.new(blocks: blocks_plain_text)
    expect(hib.plain_string).to eq("I am some text")
  end

  it "should extract text when mulitple children" do
    hib = Cms::Models::Text::RichHeader.new(blocks: blocks_with_italics)
    expect(hib.plain_string).to eq("I am some text followed by Italic text")
  end

  context "with_multiple blocks" do
    before do
      @header_with_mutliple_blocks = Cms::Models::Text::RichHeader.new(blocks: two_blocks_plain_text)
    end
