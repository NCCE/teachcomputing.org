...
RSpec.describe Cms::Providers::Strapi::GraphqlClient do
  ...
  it "calls all with query parameter adds filter" do
    stub_strapi_graphql_collection_query("blogs", Array.new(5) { Cms::Mocks::BlogComponents::Blog.generate_raw_data })
  ...
  it "raises RecordNotFound for unpublished resource" do
    stub_strapi_graphql_query("blogs", Cms::Mocks::BlogComponents::Blog.generate_raw_data(slug: "unpublished", publish_date: nil, published_at: nil))
    expect do
  ...
  it "class all and returns mapped resource" do
    stub_strapi_graphql_collection_query("blogs", Array.new(5) { Cms::Mocks::BlogComponents::Blog.generate_raw_data }, page: 1, page_size: 10)
